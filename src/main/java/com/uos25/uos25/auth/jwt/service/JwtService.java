package com.uos25.uos25.auth.jwt.service;
...
public class JwtService {
     ...
     * Optional 객체 반환
     */
    public Optional<String> extractCode(String accessToken) {
        try {
            // 토큰 유효성 검사하는 데에 사용할 알고리즘이 있는 JWT verifier builder 반환
            return Optional.ofNullable(JWT.require(Algorithm.HMAC512(secretKey))
                    .build() // 반환된 빌더로 JWT verifier 생성
                    .verify(accessToken) // accessToken을 검증하고 유효하지 않다면 예외 발생
                    .getClaim(CODE_CLAIM) // claim(Emial) 가져오기
                    .asString());
        } catch (Exception e) {
     ...
     */
    public void updateRefreshToken(String code, String refreshToken) {
